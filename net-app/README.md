...
וודא שאתה נמצא בתיקיית net-app/:
```bash
cd net-app
```
...
תיקיית ה-
api/ מכילה קבצים האחראים על התקשורת עם השרת.  קבצים אלו מטפלים ב:
...
ניקוי פרטי המשתמש מהזיכרון המקומי. 
שינוי סטטוס המשתמש בבסיס הנתונים ל"לא מחובר". 
מחיקת היסטוריית השיחות עם ה-AI עבור המשתמש. 
